...
pub fn regen_bindings(input: &str, output: &str, whitelist: Option<Vec<String>>) -> Result<bindgen::Bindings, std::io::Error> {
    ...
    .clang_arg("-U__linux__")
    .blacklist_type("u8").blacklist_type("u16").blacklist_type("u32").blacklist_type("u64");

    if process_ttf() {
        builder = builder.clang_arg("-DBINDGEN_TTF");
    }

    if process_mixer() {
        builder = builder.clang_arg("-DBINDGEN_MIXER");
    }

    if process_image() {
        builder = builder.clang_arg("-DBINDGEN_IMAGE");
    }
...
pub fn main() {
    process_bindgen("sdl2.h", "sdl2.rs", "sdl2", None);
}